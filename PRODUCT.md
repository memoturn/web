...
revised: 2026-06-10 — repositioned for the agent-memory database (memoturn/db); replaces the prior self-hostable agent-runtime product. Same day: folded in the hardening series (fail-closed auth, durable write mode, hardened Helm defaults, multi-node proof against real etcd)
